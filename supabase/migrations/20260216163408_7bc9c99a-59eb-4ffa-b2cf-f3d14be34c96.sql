...
-- Update handle_new_user to allow org invite signups (no scholar invite code required)
...
BEGIN
  ...
  IF NEW.email = 'administrativo@icca.org.br' THEN
    ...
    is_seed_admin := true;
  ELSE
    -- Check if this is an org invite signup (no scholar invite code needed)
    ...
    IF v_org_invite_token IS NOT NULL AND TRIM(v_org_invite_token) != '' THEN
      -- Verify the org invite token exists and is valid
      IF EXISTS (
        ...
        -- Org members get 'manager' role by default (will be adjusted when accepting invite)
