...
export function ReportVersionsDialog({ 
  ...
  const handleViewPdf = async (fileUrl: string, version: number) => {
    const url = await openReportPdf(fileUrl);
    if (url) {
      setPdfUrl(url);
      setPdfTitle(`Relatório ${referenceMonth} - Versão ${version}`);
                    ...
                    {version.fileUrl && (
                      <div className="flex items-center gap-1 flex-shrink-0">
                        <Button 
                          variant="ghost" 
                          size="icon"
                          onClick={() => handleViewPdf(version.fileUrl!, version.version)}
